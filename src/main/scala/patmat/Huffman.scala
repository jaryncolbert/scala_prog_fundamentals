...
object Huffman {
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table match {
      case Nil => throw new IllegalArgumentException("codeBits: char not found in table")
      case (currChar, bits) :: tail =>
        if (char == currChar) bits
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertAcc(tree: CodeTree, bits: List[Bit], table: CodeTable): CodeTable =
      tree match {
        case Leaf(char, weight) => table ::: List((char, bits))
        case Fork(left, right, forkChars, weight) =>
          val leftTable = convertAcc(left, bits ::: List(0), table)
          convertAcc(right, bits ::: List(1), leftTable)
      }
